...
function ProductCardLuxury({
  ...
  return (
    <TiltCard className="product-card group cursor-pointer" maxTilt={6} scale={1.02}>
      {/* Badge */}
      {product.badge && (
        <span className={`absolute top-3 left-3 z-20 text-[10px] tracking-widest uppercase px-3 py-1 font-medium ${
      ...
      <div className="relative aspect-[3/4] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 flex gap-2 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-luxury z-20 pointer-events-auto">
          <button
            type="button"
            onClick={handleQuickAdd}
            className={`flex-1 ${
              added ? "bg-emerald-600 text-white" : "bg-gold text-dark hover:bg-gold-light"
            } text-xs tracking-widest uppercase py-3 transition-colors flex items-center justify-center gap-1.5 font-medium shadow-lg`}
          >
            {added ? (
              <>
                <Check size={13} /> Added
          ...
          <button
            ...
            aria-label="Wishlist"
            className={`backdrop-blur-sm p-3 hover:bg-gold hover:text-dark transition-all duration-300 border border-white/10 ${
      ...
      {/* Info */}
      <Link href={`/products/${product.slug}`} className="block p-4 border-t border-gold/10">
        <h3 className="font-sans text-sm text-white/80 line-clamp-1 mb-2 group-hover:text-gold transition-colors duration-300">
          {product.name}
        </h3>
